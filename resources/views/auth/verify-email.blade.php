...
<head>
    ...
    <title>Verify Email | Pustaka Utama</title>
    ...
    <div class="w-full max-w-[960px] h-[520px] bg-white rounded-[32px] shadow-2xl flex flex-row overflow-hidden border border-white/40">
        ...
        <div class="w-[42%] bg-[#D7CBE7] p-8 flex flex-col justify-between relative overflow-hidden">
            ...
            <div class="bg-white/80 rounded-2xl p-4 border border-white/60 z-10 shadow-sm">
                <h2 class="font-extrabold text-sm text-[#2F1D4A] leading-tight">Verifikasi Email</h2>
                <p class="mt-1 text-[#593E7D] text-[11px] leading-relaxed font-medium">
                    Satu langkah lagi! Konfirmasi email Anda untuk mengaktifkan seluruh akses fitur anggota perpustakaan.
                </p>
            </div>
        </div>
        ...
        <div class="w-[58%] bg-white flex flex-col justify-center px-14 lg:px-18 relative">
            ...
            <div class="mb-5">
                <h1 class="text-3xl font-black text-gray-800 tracking-tight">Verifikasi Email</h1>
                <p class="text-gray-400 mt-1.5 text-xs leading-relaxed">
                    Terima kasih telah mendaftar! Sebelum memulai, silakan verifikasi alamat email Anda melalui tautan yang baru saja kami kirimkan. Jika tidak menerimanya, kami dapat mengirimkan ulang.
                </p>
            </div>

            @if (session('status') == 'verification-link-sent')
                <div class="mb-4 p-3 bg-green-50 border border-green-100 text-green-600 rounded-xl text-xs font-medium flex items-center gap-2">
            ...
            <div class="space-y-3">
                ...
                <form method="POST" action="{{ route('logout') }}">
                    @csrf
                    <button type="submit"
                            class="w-full py-3 rounded-xl border border-gray-200 text-gray-500 text-xs font-bold hover:bg-gray-50 hover:text-gray-700 transition-colors cursor-pointer">
                        Logout / Keluar
                    </button>
                </form>
            </div>
