<aside :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full'"
    ...
    <div class="flex items-center justify-between px-6 py-5 border-b border-purple-100">
        ...
        <button @click="sidebarOpen = false" class="lg:hidden text-gray-400 hover:text-[#6C4E97]">
    ...
    <nav class="flex-1 py-4 overflow-y-auto">
        <p class="text-[10px] font-bold text-gray-400 uppercase tracking-widest px-6 mb-3">MENU</p>

        <a href="{{ route('dashboard') }}"
           class="flex items-center gap-3 px-6 py-3 text-sm font-medium rounded-xl mx-3 transition
           {{ request()->routeIs('dashboard') ? 'bg-[#F0EBF7] text-[#6C4E97] font-semibold' : 'text-gray-600 hover:bg-gray-50' }}">
        ...
        <a href="{{ route('books.index') }}"
           class="flex items-center gap-3 px-6 py-3 text-sm font-medium rounded-xl mx-3 transition
           {{ request()->routeIs('books.*') ? 'bg-[#F0EBF7] text-[#6C4E97] font-semibold' : 'text-gray-600 hover:bg-gray-50' }}">
        ...
        <a href="{{ route('transactions.index') }}"
           class="flex items-center gap-3 px-6 py-3 text-sm font-medium rounded-xl mx-3 transition
           {{ request()->routeIs('transactions.index') || request()->routeIs('transactions.create_*') ? 'bg-[#F0EBF7] text-[#6C4E97] font-semibold' : 'text-gray-600 hover:bg-gray-50' }}">
            <i class="fa-solid fa-arrow-right-arrow-left w-5 text-center"></i>
            <span>Peminjaman</span>
        </a>

        <a href="{{ route('transactions.denda') }}"
           class="flex items-center gap-3 px-6 py-3 text-sm font-medium rounded-xl mx-3 transition
           {{ request()->routeIs('transactions.denda') ? 'bg-[#F0EBF7] text-[#6C4E97] font-semibold' : 'text-gray-600 hover:bg-gray-50' }}">
